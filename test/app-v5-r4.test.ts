...
withDb(() => {
  describe('appV5R4', () => {
    test('Bulk Upsert', async () => {
      ...
      expect(doc0V5R4?.report).toEqual({ a: 2, n: 2 });
      expect(doc0V5R4?.items).toEqual([
      ...
      expect(doc1V5R4?.report).toEqual({ a: 4, n: 4, p: 3, r: 1 });
      ...
      expect(doc2V5R4?.report).toEqual({ a: 6, n: 6, p: 6, r: 2 });
