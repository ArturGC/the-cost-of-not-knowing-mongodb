...
const worker = async (_: unknown, id: number): Promise<boolean> => {
  const bases = Array.from({ length: refs.workersTotal }).reduce<T.Base[]>((acc) => {
...
const main = async (): Promise<void> => {
  ...
  for (let count = 0; count < 100_000; count += 1) {
    const results = await Promise.all(Array.from({ length: refs.workersTotal }).map(worker));

    if (count % 10 === 0) {
      const time = new Date().toISOString().slice(11, 19);
      const total = Math.pow(refs.workersTotal, 2) * count * refs.base.batchSize;
