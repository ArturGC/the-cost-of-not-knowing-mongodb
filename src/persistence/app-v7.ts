...
const buildId = (key: number, date: Date): Buffer => {
  const id = `${buildKey(key)}${getYYYY(date)}${getSS(date)}`;
...
const buildLoopLogic = (
  ...
  const InLowerYYYYSSAndGteLowerMMDD = {
    $and: [{ $eq: ['$_id', lowerId] }, { $gte: ['$$this.k', lowerMMDD] }],
  };
  const InUpperYYYYSSAndLtUpperDD = {
    $and: [{ $eq: ['$_id', upperId] }, { $lt: ['$$this.k', upperMMDD] }],
  };
  const BetweenLowerAndUpperYYYYSS = {
  ...
  return {
    $cond: {
      if: {
        $or: [
          InLowerYYYYSSAndGteLowerMMDD,
          BetweenLowerAndUpperYYYYSS,
          InUpperYYYYSSAndLtUpperDD,
        ],
      },
      then: {
