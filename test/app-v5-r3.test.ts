...
withDb(() => {
  describe('appV5R3', () => {
    test('Bulk Upsert', async () => {
      ...
      const idDocs0 = P.appV5R3.buildId(docs0[0].key, docs0[0].date);

      await P.appV5R3.bulkUpsert(docs0);

      const doc0V5R3 = await mdb.collections.appV5R3.findOne({ _id: idDocs0 });

      expect(doc0V5R3?.report).toEqual({ a: 2, n: 2 });
      ...
      await P.appV5R3.bulkUpsert(docs1);

      const doc1V5R3 = await mdb.collections.appV5R3.findOne({ _id: idDocs1 });

      expect(doc1V5R3?.report).toEqual({ a: 4, n: 4, p: 3, r: 1 });
      expect(doc1V5R3?.items).toEqual([
        { date: new Date('2022-06-25T00:00:00.000Z'), a: 2, n: 2 },
        { date: new Date('2022-06-15T00:00:00.000Z'), a: 2, n: 2, p: 3, r: 1 },
      ]);

      const docs2 = fixtures.eventsBulkUpsert[2];
      const idDocs2 = P.appV5R3.buildId(docs2[0].key, docs2[0].date);

      await P.appV5R3.bulkUpsert(docs2);

      const doc2V5R3 = await mdb.collections.appV5R3.findOne({ _id: idDocs2 });

      expect(doc2V5R3?.report).toEqual({ a: 6, n: 6, p: 6, r: 2 });
      expect(doc2V5R3?.items).toEqual([
        { date: new Date('2022-06-25T00:00:00.000Z'), a: 2, n: 2 },
        { date: new Date('2022-06-15T00:00:00.000Z'), a: 2, n: 2, p: 3, r: 1 },
    ...
    test('Get Report', async () => {
      ...
      await P.appV5R3.bulkUpsert(docs);

      const reports = await P.appV5R3.getReports({ date, key });
