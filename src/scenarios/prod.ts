...
const worker = async (_: unknown, id: number): Promise<boolean> => {
  ...
  const eventsScenariosProd = list.reduce<T.EventsScenarios[]>((acc) => {
    const eventsScenarios = generator.getEventsScenarios(id);

    return eventsScenarios != null ? [...acc, eventsScenarios] : acc;
  }, []);

  if (eventsScenariosProd.length === 0) return false;

  await P.eventsScenariosProd.insertMany(eventsScenariosProd);
